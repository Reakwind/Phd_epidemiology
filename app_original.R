...
ui <- fluidPage(
  ...
  titlePanel(div(
    h1("780 Data Explorer", style = "color: #2c3e50; font-weight: bold;"),
    ...
    mainPanel(
      ...
      tabsetPanel(id = "main_tabs",
                  ...
                  tabPanel("Variable Distribution",
                           ...
                           fluidRow(
                             ...
                             column(4,
                                    ...
                                    div(class = "stat-box",
                                        h4("Export Options"),
                                        downloadButton("download_plot", "Download Plot", class = "btn-primary export-btn"),
                                        br(),
                                        downloadButton("download_data", "Download Data", class = "btn-secondary export-btn")
                                    )
                             )
                           )),
                  ...
                  tabPanel("Bivariate Analysis",
                           ...
                           fluidRow(
                             ...
                             column(4,
                                    ...
                                    div(class = "stat-box",
                                        h4("Export Options"),
                                        downloadButton("download_bivariate_plot", "Download Plot", class = "btn-primary export-btn"),
                                        br(),
                                        downloadButton("download_bivariate_data", "Download Data", class = "btn-secondary export-btn")
                                    )
                             )
                           )),
                  ...
                  tabPanel("Correlation Matrix",
                           ...
                           fluidRow(
                             column(8, withSpinner(plotOutput("correlation_plot", height = "600px"))),
                             column(4,
                                    div(class = "stat-box",
                                        h4("Settings"),
                                        sliderInput("corr_threshold", "Show correlations above:",
                                                   min = 0, max = 1, value = 0.3, step = 0.1),
                                        checkboxInput("show_insignificant", "Show non-significant correlations", FALSE)
                                    ),
                                    div(class = "stat-box",
                                        h4("Export"),
                                        downloadButton("download_correlation", "Download Plot", class = "btn-primary export-btn")
                                    )
                             )
                           ))
      )
...
server <- function(input, output) {
  ...
  output$conditional_sidebar <- renderUI({
    ...
    } else if (input$main_tabs == "Correlation Matrix") {
      tagList(
        h4("Correlation Settings", style = "color: #2c3e50;"),
        selectInput("corr_variables", "Select Variables:",
                    choices = numeric_variable_names,
                    selected = numeric_variable_names[1:min(10, length(numeric_variable_names))],
                    multiple = TRUE),
  ...
  output$data_overview_table <- DT::renderDataTable({
    summary_stats <- all_patient_data %>%
      summarise_all(list(
        Type = ~class(.)[1],
        Missing = ~sum(is.na(.)),
        `Missing %` = ~round(100 * sum(is.na(.)) / length(.), 1),
        Unique = ~length(unique(.))
      )) %>%
      t() %>%
      as.data.frame()

    var_names <- rep(names(all_patient_data), each = 4)
    stat_names <- rep(c("Type", "Missing", "Missing %", "Unique"), ncol(all_patient_data))
  ...
  output$correlation_plot <- renderPlot({
    ...
    corr_data <- all_patient_data[, input$corr_variables, drop = FALSE]
    ...
    cor_matrix <- cor(corr_data, method = input$corr_method)

    # Filter by threshold if specified
    if (!is.null(input$corr_threshold)) {
      cor_matrix[abs(cor_matrix) < input$corr_threshold & cor_matrix != 1] <- NA
    }
  ...
  output$download_correlation <- downloadHandler(
    filename = function() {
      paste0("correlation_matrix_", Sys.Date(), ".png")
    },
    content = function(file) {
